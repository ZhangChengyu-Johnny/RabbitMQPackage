...
/* confirm模式的生产者发给default交换机，直接连接队列 */
func (mq *ConfirmPublishMQ) QueueDeclare(queueName string, durable, autuDelete, exclusive, noWait bool, args amqp.Table) error {
...
func (mq *ConfirmPublishMQ) ConfirmPublishMessage(message, queueName string, mandatory, immediate bool) error {
	if _, ok := mq.Queues[queueName]; !ok {
