...
func TestWorkC1(t *testing.T) {
	...
	msgChan1, err := c.WorkMessageChan("C1-A", queueName1, false, nil)
	if err != nil {
		log.Println(err)
		return
	}
	msgChan2, err := c.WorkMessageChan("C1-B", queueName2, false, nil)
	if err != nil {
		log.Println(err)
		return
	}
...
func TestWorkC2(t *testing.T) {
	...
	msgChan1, err := c.WorkMessageChan("C2-A", queueName1, false, nil)
	if err != nil {
		log.Println(err)
		return
	}
	msgChan2, err := c.WorkMessageChan("C2-B", queueName2, false, nil)
	if err != nil {
		log.Println(err)
		return
	}
